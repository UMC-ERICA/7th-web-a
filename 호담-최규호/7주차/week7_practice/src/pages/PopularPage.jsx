...
const PopularPage = () => {
  ...
  const {data: movies, isPending, isError} = useQuery({
    queryFn: () => useGetMovies({category: 'popular', pageParam: 1}), //안에 parameter가 없는 경우 query: useGetMovies로 사용할 수 있는데 파라미터가 있으면 이렇게 해줘야한다.
    queryKey: ['movies', 'popular'], //여기서 movies로 주면 polular나 nowplaying이나 같은 값을 받아오므로 구분할 수 있게 한번 더 카테고리까지 써줘야 한다
    cacheTime: 10000,
    staleTime: 10000,
  })
